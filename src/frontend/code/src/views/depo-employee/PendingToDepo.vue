<script setup>
    ...
    function sendToDepo() {
        ...
        var nextBase = document.getElementById("select_base").value
        var finalList = []
        ...
        fetch('http://localhost:8080/parcel/to-distribution-base', {
            method: 'PUT',
            headers: {
    ...
    function nextPage() {
        ...
        fetch('http://localhost:8080/parcel/to-distribution-base?page=' + curr_page,{
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
...
<template>
    <h1>PARCEL PENDING TO DEPOS</h1>
    <div id="nav">
        <button id="next" @click="nextPage()">NEXT PAGE</button>
        <button disabled id="prev" @click="previousPage()">PREV PAGE</button>
        <button @click="sendToDepo()">DELIVER TO DEPO</button>
        <select id="select_base" :key="render.value">
            <option v-for="base in bases['bases']" :value="base['_id']">{{ base['baseLocation'] }}</option>
        </select>
    </div>

...
class StatisticController {
    ...
    showStatisticByBase(req, res, next) {
        if (req.get("Authorization")) {
            const userRole = jwt.verify(req.get("Authorization"), process.env.TOKEN_KEY).userRole
    ...
    showFromSenderStatistic(req, res, next) {
        if (req.get("Authorization")) {
            const userRole = jwt.verify(req.get("Authorization"), process.env.TOKEN_KEY).userRole
            const userBaseID = jwt.verify(req.get("Authorization"), process.env.TOKEN_KEY).workAt
            if (userRole == 2) {
                var statistic
